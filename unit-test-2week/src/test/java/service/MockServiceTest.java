...
public class MockServiceTest {
    ...
    // 1-1 어떤 포지션을 set해도 미드를 리턴하도록 테스트하시오.
    @Test
    ...
    // 4-2. champion 객체에서 이름을 가져오는 로직이 대 3번 이하 실행되면 Pass 하는 로직을 작성하세요.
    @Test
    ...
    public void list객체사이즈검증테스트(){
        ...
        champions.add(new Champion("조이", "미드", 3));
        champions.add(new Champion("베인", "원딜", 3));
    ...
    @Test
    public void 가짜챔피언객체return테스트(){
        when(mockService.findByName(anyString())).thenReturn(new Champion("티모", "탑", 3));
        assertThat(mockService.findByName("가렌").getHasSkinCount(), is(3));
        verify(mockRepository, times(1)).findByName(anyString());
    }
